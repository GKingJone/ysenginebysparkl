...
/*     */ 
/*     */ 
/* 131 */     TypeAdapter<T> result = new Adapter(gson, keyAndValueTypes[0], keyAdapter, keyAndValueTypes[1], valueAdapter, constructor);
/*     */     
/* 133 */     return result;
/*     */   }
/*     */   
...
/*     */       }
/*     */       
/* 167 */       Map<K, V> map = (Map)this.constructor.construct();
/*     */       
/* 169 */       if (peek == JsonToken.BEGIN_ARRAY) {
/* 170 */         in.beginArray();
/* 171 */         while (in.hasNext()) {
/* 172 */           in.beginArray();
/* 173 */           K key = this.keyTypeAdapter.read(in);
/* 174 */           V value = this.valueTypeAdapter.read(in);
...
/* 186 */           K key = this.keyTypeAdapter.read(in);
/* 187 */           V value = this.valueTypeAdapter.read(in);
...
/* 205 */         out.beginObject();
/* 206 */         for (Entry<K, V> entry : map.entrySet()) {
...
/* 215 */       List<JsonElement> keys = new ArrayList(map.size());
/*     */       
/* 217 */       List<V> values = new ArrayList(map.size());
/* 218 */       for (Entry<K, V> entry : map.entrySet()) {
...
/* Location:              E:\BaiduYunDownload\伊萨时期的一些代码\SparkEngine终极版\jars\YISAEngineBySpark2-4.0.6-SNAPSHOT-jar-with-dependencies.jar!\com\google\gson\internal\bind\MapTypeAdapterFactory.class

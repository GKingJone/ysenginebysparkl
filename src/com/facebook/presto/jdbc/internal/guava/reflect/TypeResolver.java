...
/* 106 */     new TypeVisitor()
/*     */     {
/* 108 */       void visitTypeVariable(TypeVariable<?> typeVariable) { this.val$mappings.put(new TypeVariableKey(typeVariable), to); }
/*     */       
...
/* 121 */           TypeResolver.populateTypeMappings(this.val$mappings, fromUpperBounds[i], toUpperBounds[i]);
/*     */         }
/* 123 */         for (int i = 0; i < fromLowerBounds.length; i++)
/* 124 */           TypeResolver.populateTypeMappings(this.val$mappings, fromLowerBounds[i], toLowerBounds[i]);
/*     */       }
/*     */       
...
/* 136 */           TypeResolver.populateTypeMappings(this.val$mappings, fromArgs[i], toArgs[i]);
/*     */       }
/*     */       
...
/* 142 */         TypeResolver.populateTypeMappings(this.val$mappings, fromArrayType.getGenericComponentType(), componentType);
/*     */       }
/*     */       
/*     */ 
/*     */       void visitClass(Class<?> fromClass)
/*     */       {
/* 148 */         String str = String.valueOf(String.valueOf(fromClass));throw new IllegalArgumentException(21 + str.length() + "No type mapping from " + str); } }.visit(new Type[] { from });
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/* 209 */       String str1 = String.valueOf(String.valueOf(arg));String str2 = String.valueOf(String.valueOf(type.getSimpleName()));throw new IllegalArgumentException(10 + str1.length() + str2.length() + str1 + " is not a " + str2);
/*     */     }
/*     */   }
/*     */   
...
/*     */     {
/* 227 */       Builder<TypeVariableKey, Type> builder = ImmutableMap.builder();
/* 228 */       builder.putAll(this.map);
/* 229 */       for (Entry<TypeVariableKey, ? extends Type> mapping : mappings.entrySet()) {
/* 230 */         TypeVariableKey variable = (TypeVariableKey)mapping.getKey();
...
/*     */     {
/* 261 */       Type type = (Type)this.map.get(new TypeVariableKey(var));
/* 262 */       if (type == null) {
...
/*     */   {
/* 310 */     private static final WildcardCapturer wildcardCapturer = new WildcardCapturer(null);
/*     */     
...
/*     */       }
/*     */       
/* 360 */       for (Type t = arg; t != null; t = (Type)this.mappings.get(TypeVariableKey.forLookup(t))) {
/* 361 */         if (var.equalsType(t))
/*     */         {
/*     */ 
/*     */ 
/*     */ 
/* 366 */           for (Type x = arg; x != null; x = (Type)this.mappings.remove(TypeVariableKey.forLookup(x))) {}
/* 367 */           return;
/*     */         }
/*     */       }
...
/* 409 */           int i = this.id.incrementAndGet();String str1 = String.valueOf(String.valueOf(Joiner.on('&').join(upperBounds)));String name = 33 + str1.length() + "capture#" + i + "-of ? extends " + str1;
/*     */           
...
/* Location:              E:\BaiduYunDownload\伊萨时期的一些代码\SparkEngine终极版\jars\YISAEngineBySpark2-4.0.6-SNAPSHOT-jar-with-dependencies.jar!\com\facebook\presto\jdbc\internal\guava\reflect\TypeResolver.class

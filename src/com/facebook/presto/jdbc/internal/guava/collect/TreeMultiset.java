...
/*     */   {
/*  74 */     return new TreeMultiset(Ordering.natural());
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */   {
/*  91 */     return comparator == null ? new TreeMultiset(Ordering.natural()) : new TreeMultiset(comparator);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/* 127 */     this.rootReference = new Reference(null);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */ 
/*     */ 
/*     */     abstract int nodeAggregate(AvlNode<?> paramAvlNode);
/*     */     
/*     */ 
...
/*     */   {
/* 162 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
...
/*     */     {
/* 231 */       E e = (E)element;
/* 232 */       AvlNode<E> root = (AvlNode)this.rootReference.get();
...
/* 251 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
/* 252 */     if (root == null) {
/* 253 */       comparator().compare(element, element);
/* 254 */       AvlNode<E> newRoot = new AvlNode(element, occurrences);
/* 255 */       successor(this.header, newRoot, this.header);
/* 256 */       this.rootReference.checkAndSet(root, newRoot);
/* 257 */       return 0;
/*     */     }
...
/*     */     }
/* 271 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
...
/*     */     }
/*     */     
/* 298 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
/* 299 */     if (root == null) {
/* 300 */       if (count > 0) {
...
/*     */     
/* 317 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
/* 318 */     if (root == null) {
/* 319 */       if (oldCount == 0) {
/* 320 */         if (newCount > 0) {
...
/* 335 */     new Multisets.AbstractEntry()
/*     */     {
/*     */       public E getElement() {
/* 338 */         return (E)baseEntry.getElement();
/*     */       }
/*     */       
/*     */       public int getCount()
/*     */       {
...
/*     */   {
/* 357 */     AvlNode<E> root = (AvlNode)this.rootReference.get();
/* 358 */     if (root == null) {
/* 359 */       return null;
/*     */     }
/*     */     AvlNode<E> node;
...
/* 364 */       AvlNode<E> node = ((AvlNode)this.rootReference.get()).ceiling(comparator(), endpoint);
/* 365 */       if (node == null) {
/* 366 */         return null;
/*     */       }
...
/*     */   
/*     */   @Nullable
/* 379 */   private AvlNode<E> lastNode() { AvlNode<E> root = (AvlNode)this.rootReference.get();
/* 380 */     if (root == null) {
/* 381 */       return null;
/*     */     }
/*     */     AvlNode<E> node;
...
/* 386 */       AvlNode<E> node = ((AvlNode)this.rootReference.get()).floor(comparator(), endpoint);
/* 387 */       if (node == null) {
/* 388 */         return null;
/*     */       }
...
/*     */   {
/* 402 */     new Iterator() {
/* 403 */       AvlNode<E> current = TreeMultiset.this.firstNode();
...
/*     */   {
/* 444 */     new Iterator() {
/* 445 */       AvlNode<E> current = TreeMultiset.this.lastNode();
...
/*     */ 
/*     */ 
/* 501 */   static int distinctElements(@Nullable AvlNode<?> node) { return node == null ? 0 : node.distinctElements; }
/*     */   
...
/*     */     
/*     */     @Nullable
/* 508 */     public T get() { return (T)this.value; }
/*     */     
...
/* 789 */         AvlNode<E> newTop = this.pred;
/*     */         
/* 791 */         newTop.left = this.left.removeMax(newTop);
/* 792 */         newTop.right = this.right;
/* 793 */         this.distinctElements -= 1;
...
/*     */     
/*     */ 
/* 896 */     private static int height(@Nullable AvlNode<?> node) { return node == null ? 0 : node.height; }
/*     */     
/*     */     @Nullable
/*     */     private AvlNode<E> ceiling(Comparator<? super E> comparator, E e) {
/* 900 */       int cmp = comparator.compare(e, this.elem);
/* 901 */       if (cmp < 0)
/* 902 */         return this.left == null ? this : (AvlNode)MoreObjects.firstNonNull(this.left.ceiling(comparator, e), this);
/* 903 */       if (cmp == 0) {
/* 904 */         return this;
/*     */       }
...
/* 912 */       if (cmp > 0)
/* 913 */         return this.right == null ? this : (AvlNode)MoreObjects.firstNonNull(this.right.floor(comparator, e), this);
/* 914 */       if (cmp == 0) {
/* 915 */         return this;
/*     */       }
...
/*     */     
/*     */ 
/* 969 */     Comparator<? super E> comparator = (Comparator)stream.readObject();
/* 970 */     Serialization.getFieldSetter(AbstractSortedMultiset.class, "comparator").set(this, comparator);
/* 971 */     Serialization.getFieldSetter(TreeMultiset.class, "range").set(this, GeneralRange.all(comparator));
/*     */     
/*     */ 
/* 974 */     Serialization.getFieldSetter(TreeMultiset.class, "rootReference").set(this, new Reference(null));
/*     */     
/*     */ 
/* 977 */     AvlNode<E> header = new AvlNode(null, 1);
...
/* Location:              E:\BaiduYunDownload\伊萨时期的一些代码\SparkEngine终极版\jars\YISAEngineBySpark2-4.0.6-SNAPSHOT-jar-with-dependencies.jar!\com\facebook\presto\jdbc\internal\guava\collect\TreeMultiset.class

...
/*      */ 
/*      */ 
/* 1035 */       this._numberBigDecimal = NumberInput.parseBigDecimal(getText());
/* 1036 */     } else if ((this._numTypesValid & 0x4) != 0) {
/* 1037 */       this._numberBigDecimal = new BigDecimal(this._numberBigInt);
/* 1038 */     } else if ((this._numTypesValid & 0x2) != 0) {
/* 1039 */       this._numberBigDecimal = BigDecimal.valueOf(this._numberLong);
/* 1040 */     } else if ((this._numTypesValid & 0x1) != 0) {
/* 1041 */       this._numberBigDecimal = BigDecimal.valueOf(this._numberInt);
/*      */     } else {
/* 1043 */       _throwInternal();
/*      */     }
/* 1045 */     this._numTypesValid |= 0x10;
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/*      */     throws JsonParseException
/*      */   {
/* 1055 */     String msg = "Unexpected character (" + _getCharDesc(ch) + ") in numeric value";
/* 1056 */     if (comment != null) {
/* 1057 */       msg = msg + ": " + comment;
/*      */     }
/* 1059 */     _reportError(msg);
/*      */   }
/*      */   
/*      */   protected void reportInvalidNumber(String msg) throws JsonParseException {
/* 1063 */     _reportError("Invalid numeric value: " + msg);
/*      */   }
/*      */   
/*      */   protected void reportOverflowInt() throws IOException {
/* 1067 */     _reportError(String.format("Numeric value (%s) out of range of int (%d - %s)", new Object[] { getText(), Integer.valueOf(Integer.MIN_VALUE), Integer.valueOf(Integer.MAX_VALUE) }));
/*      */   }
/*      */   
/*      */   protected void reportOverflowLong() throws IOException
/*      */   {
/* 1072 */     _reportError(String.format("Numeric value (%s) out of range of long (%d - %s)", new Object[] { getText(), Long.valueOf(Long.MIN_VALUE), Long.valueOf(Long.MAX_VALUE) }));
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/* 1106 */     if (bits < 0) {
/* 1107 */       throw reportInvalidBase64Char(b64variant, unescaped, index);
/*      */     }
/* 1109 */     return bits;
/*      */   }
/*      */   
...
/*      */   {
/* 1114 */     if (ch != '\\') {
/* 1115 */       throw reportInvalidBase64Char(b64variant, ch, index);
/*      */     }
/* 1117 */     char unescaped = _decodeEscaped();
/*      */     
/* 1119 */     if ((unescaped <= ' ') && 
/* 1120 */       (index == 0)) {
/* 1121 */       return -1;
...
/*      */     
/*      */ 
/* 1125 */     int bits = b64variant.decodeBase64Char(unescaped);
/* 1126 */     if (bits < 0) {
/* 1127 */       throw reportInvalidBase64Char(b64variant, unescaped, index);
/*      */     }
/* 1129 */     return bits;
/*      */   }
/*      */   
...
/* 1133 */     return reportInvalidBase64Char(b64variant, ch, bindex, null);
/*      */   }
/*      */   
/*      */ 
...
/*      */     String base;
/* 1142 */     if (ch <= 32) {
/* 1143 */       base = "Illegal white space character (code 0x" + Integer.toHexString(ch) + ") as character #" + (bindex + 1) + " of 4-char base64 unit: can only used between units"; } else { String base;
/* 1144 */       if (b64variant.usesPaddingChar(ch)) {
/* 1145 */         base = "Unexpected padding character ('" + b64variant.getPaddingChar() + "') as character #" + (bindex + 1) + " of 4-char base64 unit: padding only legal as 3rd or 4th character"; } else { String base;
/* 1146 */         if ((!Character.isDefined(ch)) || (Character.isISOControl(ch)))
/*      */         {
/* 1148 */           base = "Illegal character (code 0x" + Integer.toHexString(ch) + ") in base64 content";
/*      */         } else
/* 1150 */           base = "Illegal character '" + (char)ch + "' (code 0x" + Integer.toHexString(ch) + ") in base64 content";
/*      */       } }
/* 1152 */     if (msg != null) {
/* 1153 */       base = base + ": " + msg;
/*      */     }
/* 1155 */     return new IllegalArgumentException(base);
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/*      */     throws IOException
/*      */   {
/* 1167 */     if (!loadMore()) _reportInvalidEOF();
/*      */   }
/*      */   
/*      */   @Deprecated
/* 1171 */   protected boolean loadMore() throws IOException { return false; }
/*      */   
...
/* Location:              E:\BaiduYunDownload\伊萨时期的一些代码\SparkEngine终极版\jars\YISAEngineBySpark2-4.0.6-SNAPSHOT-jar-with-dependencies.jar!\com\facebook\presto\jdbc\internal\jackson\core\base\ParserBase.class
